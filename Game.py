...
class Raquete:
    def __init__(self, tamanho):
        ...
        self.imagem.fill(VERDE) #Raquete cor verde
        self.imagem_retangulo = self.imagem.get_rect()
        self.imagem_retangulo[0] = 0 #Raquete na posição x = 0

    def move(self, y):
        self.imagem_retangulo[1] += y * 10 #Raquete se move no eixo y

        global posicaoYraquete
        posicaoYraquete = self.imagem_retangulo.centery #atualiza a posição global y do centro da raquete

    def atualiza(self, tecla):
        if tecla > 0.5:
            self.move(-1) #Move raquete pra cima se a tecla > 0.5
        elif tecla < 0.5:
            self.move(1) #Move raquete pra baixo se a tecla < 0.5
        self.imagem_retangulo.clamp_ip(tela_retangulo)

    def realiza(self):
        tela.blit(self.imagem, self.imagem_retangulo) #Mostrar a raquete
...
class Bola:
    def __init__(self, tamanho):
        ...
        self.imagem = pygame.Surface(tamanho)
        self.imagem.fill(VERMELHO) #Preenche a bola de vermelho
        self.imagem_retangulo = self.imagem.get_rect() #Colisor em torno da bola
        self.setBola()
        global erro
        self.erro = 0
    ...
    def setBola(self):
        x = self.aleatorio() #Posição aleatória de spawn da bolinha
        y = self.aleatorio()
...
class RedeNeural(Thread):
    def __init__(self, YRaquete, XBolinha, YBola, bias = -1):
        ...
        global pesosPrimeiroNeuronioCamadaEntrada, pesosSegundoNeuronioCamadaEntrada, pesosPrimeiroNeuronioCamadaOculta, pesosSegundoNeuronioCamadaOculta, pesosNeuronioDeSaida
    ...
    def feedforward(self):
        ...
        self.saidaPrimeiroNeuronioCamadaOculta = round(self.tangenteHiperbolica(np.sum(np.array([self.saidaPrimeiroNeuronioCamadaEntrada, self.saidaSegundoNeuronioCamadaEntrada])*self.pesosPrimeiroNeuronioCamadaOculta)), 6)
        self.saidaSegundoNeuronioCamadaOculta = round(self.tangenteHiperbolica(np.sum(np.array([self.saidaPrimeiroNeuronioCamadaEntrada, self.saidaSegundoNeuronioCamadaEntrada])*self.pesosSegundoNeuronioCamadaOculta)), 6)
